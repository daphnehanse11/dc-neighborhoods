...
export default function Map({ mode, onPolygonChange, address, isDrawing, drawTrigger, onClearDrawing }: MapProps) {
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: import.meta.env.VITE_MAPLIBRE_STYLE || 'https://basemaps.cartocdn.com/gl/positron-gl-style/style.json',
      center: DC_CENTER,
      zoom: DC_ZOOM,
      maxBounds: DC_BOUNDS,
      minZoom: 10,
  ...
  useEffect(() => {
    if (drawTrigger > 0 && drawRef.current && mapReady) {
      console.log('Starting draw mode, trigger:', drawTrigger)
  ...
  useEffect(() => {
    if (!isDrawing && drawRef.current) {
      // If we stopped drawing without a polygon, clear
    }
  }, [isDrawing])
